...
$("#stop-note").click(function () {
  ...
  recognition.onresult = function (event) {
    ...
    $("#note-text").append("<b><i>Person 1: </i></b> " + noteContent);
    noteContent = '';
  };
  $("#note").attr("disabled", false);
  $("#stop-note").attr("disabled", true);
...
// Get the note text area element
const noteTextArea = document.getElementById('note-text');

// Get the "Save Notes" button element
const saveNotesButton = document.getElementById('save-note');

// Add a click event listener to the "Save Notes" button
saveNotesButton.addEventListener('click', () => {
  // Get the notes text from the note text area
  const notes = noteTextArea.innerText;
   
  // Create a new Blob object with the notes text
  const blob = new Blob([notes], {type: "text/plain;charset=utf-8"});
  const notesWithLineBreaks = notes.replace(/\n/g, '\r\n');
  // Save the Blob object as a file with the name "notes.txt"
